...
def main():
    ...
    success_unit = run_command(
        "python -m pytest tests/unit/ -v --tb=short", "Tests unitaires"
    )

    # 2. Tests d'intégration
    print("\n📋 2. TESTS D'INTÉGRATION")
    success_integration = run_command(
        "python -m pytest tests/integration/ -v --tb=short", "Tests d'intégration"
    )

    # 3. Tests fonctionnels
    print("\n📋 3. TESTS FONCTIONNELS")
    success_functional = run_command(
        "python -m pytest tests/functional/ -v --tb=short", "Tests fonctionnels"
    )
